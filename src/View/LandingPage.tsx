...
const LandingPage = ({ handleErrorReload, error, jobs, loadMore, initialLoading, loading, noMoreItems, page }: LandingPageProps) => {
    ...
    return (
        <div className="flex flex-col min-h-screen" style={{ backgroundColor: "#fcf7f2" }}>
            ...
            {!noMoreItems && <button disabled={loading === true} onClick={() => loadMore(page + 1)} className="flex justify-center items-center bg-indigo-500 p-3 mb-4 w-36 self-center focus:outline-none hover:bg-indigo-700 rounded-xl text-white transition-colors duration-300 ">{loading ? "Loading" : "Load more"}
